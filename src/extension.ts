...
export function activate(context: vscode.ExtensionContext) {
  ...
  console.log('Congratulations, your extension "snipe-vscode" is now active!');
  ...
  let disposable = vscode.commands.registerCommand(
    ...
    async () => {
      ...
      // Collect all tabs from all groups
      tabGroups.all.forEach((group) => {
        group.tabs.forEach((tab, index) => {
          ...
          allTabs.push({
            label: shortcut ? `${shortcut}: ${label}` : label,
            description,
            tabGroup: group,
            tab,
            shortcut,
      ...
      quickPick.placeholder = "Select tab to switch to...";
      quickPick.matchOnDescription = true;

      // Handle keyboard shortcuts
      quickPick.onDidChangeValue((value) => {
        const shortcutTab = allTabs.find(
          (tab) => tab.shortcut === value.toLowerCase()
        );
        if (shortcutTab) {
          quickPick.hide();
